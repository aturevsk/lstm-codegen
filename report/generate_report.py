...
def build():
    ...
    story.append(CalloutBox([
        ...
        "ALL four options produce IDENTICAL predictions: 100% match across 201 inputs / 15,075 timesteps",
    ...
    # ── §3.5  Import and Codegen Challenges for Options 3 & 4 ───────────────
    story.append(SubHeader("3.5  Import and Codegen Challenges for Options 3 and 4"))
    story += [sp(6)]
    story.append(p(
        "Options 3 and 4 required resolving two distinct technical obstacles before "
        "MATLAB Coder could generate deployable C code."
    ))
    story += [sp(4)]

    # Challenge 1
    story.append(p("<b>Challenge 1: Auto-generated custom argmax layer not codegen-compatible</b>"))
    story.append(p(
        "When <font name='Courier'>importNetworkFromPyTorch</font> (Option 3) or "
        "<font name='Courier'>importNetworkFromONNX</font> (Option 4) imports the model, "
        "it faithfully recreates the entire computation graph — including the final "
        "<font name='Courier'>argmax</font> operation. "
        "Because PyTorch's <font name='Courier'>torch.export</font> bakes argmax into the "
        "exported graph as a custom op, MATLAB generates a custom layer class "
        "(<font name='Courier'>LSTMModel_max_to_3</font>) stored in the auto-generated "
        "<font name='Courier'>+LSTMSeqToSeqModel/</font> package directory. "
        "This custom layer does <b>not implement the codegen interface</b> — it lacks the "
        "<font name='Courier'>getCoderAttributes()</font> method required by MATLAB Coder "
        "to lower the layer to C. Running <font name='Courier'>codegen()</font> on the "
        "full imported network fails immediately."
    ))
    story += [sp(4)]
    story.append(p("<b>Resolution:</b> strip the custom layer and move argmax to the entry-point wrapper."))
    story.append(Paragraph(
        "% Keep only the three standard codegen-compatible layers:\n"
        "% SequenceInputLayer, LSTMLayer, FullyConnectedLayer\n"
        "% Drop LSTMModel_max_to_3 (custom, not codegen-safe)\n"
        "netCodegen = dlnetwork(layerGraph(stdLayers));   % 3-layer net\n\n"
        "% Entry-point applies argmax in plain MATLAB (outside codegen boundary):\n"
        "[~, idx] = max(Ydata, [], 1);          % max over class axis\n"
        "pred = int32(idx) - int32(1);          % 1-indexed -> 0-indexed",
        T_CODE))
    story.append(p(
        "This approach works here because argmax is at the <b>output boundary</b> of the network. "
        "If the custom layer were in the middle of the network, a different strategy would be "
        "required — either writing a codegen-compatible replacement custom layer implementing "
        "<font name='Courier'>getCoderAttributes()</font>, or using the MLIR/TOSA path "
        "(Option 2) which bypasses the Deep Learning Toolbox layer system entirely."
    ))
    story += [sp(6)]

    # Challenge 2
    story.append(p("<b>Challenge 2: FC bias absent in ONNX export (Option 4 only)</b>"))
    story.append(p(
        "When the PyTorch model is exported to ONNX and then imported via "
        "<font name='Courier'>importNetworkFromONNX</font>, the FC (fully connected) layer "
        "bias is not correctly recovered. "
        "The ONNX graph represents the FC operation as a <font name='Courier'>Gemm</font> "
        "or <font name='Courier'>MatMul + Add</font> node depending on the opset and export "
        "configuration. MATLAB's ONNX importer maps the weight matrix (stored under the "
        "parameter name <font name='Courier'>'MatMul'</font>) correctly, but does not "
        "surface the bias as the <font name='Courier'>Bias</font> property of a "
        "<font name='Courier'>fullyConnectedLayer</font>. "
        "The root cause is in how PyTorch's ONNX exporter encodes the bias node, "
        "not a defect in <font name='Courier'>importNetworkFromONNX</font> itself."
    ))
    story += [sp(4)]
    story.append(p(
        "<b>Resolution:</b> LSTM weights (InputWeights, RecurrentWeights, Bias) are taken "
        "from ONNX learnables; the FC bias is sourced from the .pt2 reference via "
        "<font name='Courier'>importNetworkFromPyTorch</font>. "
        "The dlnetwork is then rebuilt entirely from scratch using layer constructors "
        "with weights passed explicitly — not using the auto-imported network directly:"
    ))
    story.append(Paragraph(
        "% LSTM weights from ONNX; FC bias from PT2 fallback\n"
        "newLSTM = lstmLayer(50, 'OutputMode', 'sequence', ...\n"
        "    'InputWeights',     single(onnxIW), ...\n"
        "    'RecurrentWeights', single(onnxRW), ...\n"
        "    'Bias',             single(onnxB));\n"
        "newFC = fullyConnectedLayer(5, ...\n"
        "    'Weights', single(fcW), ...    % from ONNX MatMul param\n"
        "    'Bias',    single(fcB));        % from PT2 reference\n"
        "netCodegen = dlnetwork(layerGraph([sequenceInputLayer(3); newLSTM; newFC]));",
        T_CODE))
    story.append(p(
        "The 200-test boundary accuracy validation (Section 4.3) confirmed that the "
        "FC bias was correctly transferred despite this workaround — Options 3 and 4 "
        "produce bit-exact identical predictions to Option 1 across all 15,000 tested timesteps. "
        "Option 4 is therefore best described as an <b>ONNX-assisted import with PT2 fallback "
        "for the missing FC bias</b>, rather than a fully self-contained ONNX pipeline."
    ))
    story += [sp(6)]

    # Architectural note
    story.append(p("<b>Architectural note: why Option 2 does not have these problems</b>"))
    story.append(p(
        "Option 2 uses the MATLAB Coder Support Package for PyTorch, which lowers the "
        ".pt2 graph through <b>MLIR/TOSA</b> (Tensor Operator Set Architecture) — a "
        "low-level primitive IR — before MATLAB Coder generates C. "
        "At the TOSA level there are no 'layers' and no custom classes: the argmax becomes "
        "<font name='Courier'>tosa.argmax</font>, activations become "
        "<font name='Courier'>tosa.sigmoid</font>/<font name='Courier'>tosa.tanh</font>, etc. "
        "Because every op has a TOSA primitive equivalent, the custom-layer codegen problem "
        "cannot arise. Options 3 and 4, by contrast, use the Deep Learning Toolbox "
        "dlnetwork representation, which operates at a much higher abstraction level and "
        "requires every operation to map to a built-in or codegen-compatible custom layer."
    ))
    ...
    story.append(p(
        "Accuracy was validated in <b>two phases</b>: an initial single-input sanity check, "
        "followed by a 200-test boundary sweep specifically targeting inputs that produce "
        "non-Class-0 predictions — the harder cases where logit margins are narrower and "
        "floating-point differences are more likely to cause disagreement."
    ))
    story += [sp(5)]

    story.append(p("<b>Phase 1 — Single seed-42 input (initial check)</b>"))
    story.append(p(
        "All four implementations were verified against the PyTorch ExportedProgram reference "
        "on the seed-42 test input (the same input used for timing). "
        "Predictions for all 75 timesteps were identical across all options:"
    ))
    story.append(Paragraph(
        "t=0: class 3   t=1..74: class 0, 0, 0, ..., 0  (75/75 match)",
        T_CODE))
    story.append(p(
        "Class 3 at t=0 is a transient from zero hidden-state initialization. "
        "All 74 subsequent timesteps predict class 0."
    ))
    story += [sp(5)]

    story.append(p("<b>Phase 2 — 200 boundary inputs (stronger validation)</b>"))
    story.append(p(
        "To move beyond a single-input check, a boundary sweep was run using "
        "<font name='Courier'>benchmark/accuracy_test_boundary.py</font>. "
        "The script generates random inputs with varying scales (0.5x, 1x, 2x, 3x, 5x "
        "standard normal) and sweeps them through Option 1, collecting the first 200 inputs "
        "that produce <b>at least one non-Class-0 prediction</b>. "
        "These boundary inputs were then run through all four C implementations and predictions compared. "
        "422 random inputs were swept to find the 200 qualifying cases."
    ))
    story += [sp(4)]

    # Results table — Phase 2
    boundary_data = [
        ["Test Phase","Inputs","Timesteps","Non-Class-0 preds","All 4 options match","Result"],
        ["Phase 1: seed-42",     "1",   "75",    "1 (1.3%)", "75 / 75",       "PASS"],
        ["Phase 2: boundary sweep", "200","15,000","271 (1.8%)", "15,000 / 15,000","PASS"],
        ["Combined",             "201","15,075","272 (1.8%)", "15,075 / 15,075","PASS"],
    ]
    bt = Table(boundary_data, colWidths=[3.5*cm, 1.2*cm, 1.8*cm, 2.8*cm, 3.0*cm, 1.5*cm])
    bt.setStyle(TableStyle(HDR_STYLE + [
        ("BACKGROUND",(5,1),(5,2),colors.HexColor("#E8F5E9")),
        ("TEXTCOLOR", (5,1),(5,2),GREEN),
        ("FONTNAME",  (5,1),(5,3),"Helvetica-Bold"),
        ("BACKGROUND",(0,3),(-1,3),colors.HexColor("#EAF6FF")),
    ...
    story.append(p(
        "Class distribution across the 200 boundary tests (Option 1 as reference). "
        "The model is heavily saturated toward Class 0; non-Class-0 predictions appear "
        "only in isolated timesteps within a sequence:"
    ))
    class_dist = [
        ["Class","Count / 15,000 timesteps","Percentage","Note"],
        ["0","14,729","98.2%","dominant across all inputs"],
        ["1","78","0.5%",""],
        ["2","95","0.6%","most common non-zero class"],
        ["3","41","0.3%",""],
        ["4","57","0.4%",""],
    ]
    ct = Table(class_dist, colWidths=[1.5*cm, 4.0*cm, 2.5*cm, 5.8*cm])
    ct.setStyle(TableStyle(HDR_STYLE + [
        ("BACKGROUND",(0,1),(-1,1),colors.HexColor("#E8F5E9")),
    ]))
    story += [ct, sp(6)]

    story.append(p(
        "<b>Conclusion:</b> All four C implementations produce bit-exact identical int32 predictions "
        "across 15,075 total timesteps (201 distinct input sequences), including 272 non-Class-0 "
        "predictions where logit margins are narrowest. No disagreements were found. "
        "The FC bias workaround in Option 4 (Section 3.5) and the argmax-stripping in "
        "Options 3 and 4 introduced no numerical error."
    ))
    story += [sp(4)]

    # Original per-option table retained for reference
    verify_data = [
        ["Option","Implementation","Phase 1\n(1 input)","Phase 2\n(200 inputs)","Status"],
        ["Ref",  "PyTorch ExportedProgram","--","--","REFERENCE"],
        ["1","Hand-written C (gate-split weights)","75/75","15,000/15,000","PASS"],
        ["2","MATLAB Coder via PyTorch Pkg (MLIR/TOSA)","75/75","15,000/15,000","PASS"],
        ["3","MATLAB Coder dlnetwork from PyTorch","75/75","15,000/15,000","PASS"],
        ["4","MATLAB Coder dlnetwork from ONNX","75/75","15,000/15,000","PASS"],
    ]
    vt = Table(verify_data, colWidths=[1.0*cm, 5.8*cm, 2.2*cm, 3.2*cm, 1.6*cm])
